...
for file in files:
    ...
    df["label"] = df.apply(lambda row: row[row == 1].index.to_list(), axis=1)
    new_df = pd.DataFrame()
    new_df["text"] = df["text"]
    new_df["label"] = df["label"]
    new_df["category"] = df.apply(lambda row: get_category(row["label"]), axis=1)
    new_df.to_csv("./dataset/"+file)
